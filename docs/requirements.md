...
```jsonc
{
  ...
  "claudeCodexStatus.warningRemainingPercent": 30, // 残量がこの%未満でバーに⚠+ホバー行を黄文字。0で無効
  "claudeCodexStatus.criticalRemainingPercent": 10 // 残量がこの%未満でバーにエラーアイコン+ホバー行を赤文字。0で無効
}
```
