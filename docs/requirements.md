...
- **トークンの有効期限とリフレッシュ**: `accessToken` は短命で `claudeAiOauth.expiresAt`(epochミリ秒)を持つ。切れると `refreshToken` での更新が必要だが、本拡張機能は自前で更新せず、Claude Code CLI / 公式拡張機能が起動時に自動更新して `.credentials.json` を書き換えるのに追従する(次回ポーリングで新トークンを読み直す)。書き戻しの競合・破損リスクを避けるため、拡張機能側からのファイル更新は行わない。失効中は取得に失敗するため、`expiresAt` を事前に見て `tokenExpired` として「更新待ち」を明示する
...
- ホバー時に以下を表示:
  ...
  - 最終取得時刻
  - クリックでの再取得の案内
- どの項目のツールチップにも全プロバイダの詳細を載せるが、ホバーした項目のプロバイダを先頭に置く
- **制約**: 隣接する項目のツールチップ本文(Markdown文字列)を同一にしてはならない。VSCode はホバーの同一性を本文文字列で判定し(hoverService の `getHoverIdFromContent`)、表示中と同一内容の表示要求を無視するため、同一だと項目間の移動でホバーが表示されなくなる
...
| `tokenExpired` | トークンの有効期限(`expiresAt`)切れ。Claude Code 未起動で未更新 | `トークン更新待ち` + `$(key)` | 出さない |
