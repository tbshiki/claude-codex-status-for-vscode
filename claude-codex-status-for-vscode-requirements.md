...
- Codex CLI の5時間枠・週枠(または相当する枠)の利用率とリセット時刻をステータスバーに表示
...
- 利用率データ: `~/.codex/logs_1.sqlite`(または相当するログファイル)に記録された直近のAPIレスポンスヘッダ(`x-codex-*` 系)から抽出する方式を検討。代替として `codex app-server` へのJSON-RPC呼び出し(`account/rateLimits/read`)も候補とする
...
| M2 | Codex対応追加(ログ解析 or app-server連携) |
| M3 | 設定項目の拡充・エラーハンドリング強化 |
| M4 | Marketplace公開判断(公開する場合はpublisher登録・アイコン・README整備) |

## 8. オープンな論点(要議論)

- Codexの利用率取得方式は「ログファイル受動監視」と「`app-server`への能動的問い合わせ」のどちらを採用するか
